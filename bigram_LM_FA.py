...
class BigramLanguageModel_FA(nn.Module):
    def __init__(self, vocab_size, n_embed=384, num_heads=6, block_size=256):
        ...
        self.blocks = nn.Sequential(Block(n_embed, num_heads, block_size), 
    ...
    def generate(self, idx, max_new_tokens): #idx is B,T
        for _ in range(max_new_tokens):
            idx_cond = idx[:, -self.block_size:] #this ensures that the input to the model is always of shape (B, block_size), which is the maximum context size that the model can handle
